...
import { RENTARA_MAP_MAX_NATIVE_ZOOM, RENTARA_MAP_MAX_ZOOM, RENTARA_MAP_PRIMARY, RENTARA_MAP_TILE_URL } from '@/constants/rentaraMapStyle'
import {
...
import VehicleHeroImage from '@/components/media/VehicleHeroImage'
...
export type ExploreRentalsMapInnerProps = {
  ...
  /**
   * Leaflet zoom control corner. Mobile `/map` uses `bottomright` so filters at the top stay clear.
   */
...
/** Zoom after selecting a pin — road-level detail (Esri native tiles to ~18). */
const SELECTED_PIN_STREET_ZOOM = 18

/**
 * Leaflet popup / fly skips: map maxZoom is {@link RENTARA_MAP_MAX_ZOOM} while we cap fly at 18 —
...
/**
 * Leaflet map: Esri street basemap, RentaraH price tags + density clusters; styled popups.
    ...
    <Stack direction="column" sx={{ height: '100%', minHeight: 0 }} spacing={0}>
      <Box sx={{ flex: 1, minHeight: 0, position: 'relative' }}>
        ...
        >
          <TileLayer
            attribution=""
            url={RENTARA_MAP_TILE_URL}
            maxZoom={RENTARA_MAP_MAX_ZOOM}
            maxNativeZoom={RENTARA_MAP_MAX_NATIVE_ZOOM}
          />          <MapLayoutStabilizer />
