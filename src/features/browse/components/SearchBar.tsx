...
export default function SearchBar({
  ...
  const ariaLabel = hasCompact
    ? t('search.editSearchAria', { summary: compactSummaryLine ?? '' })
    : t('search.editSearchAriaParts', { location: locationLabel, dates: datesLabel })
  ...
  return (
    ...
    >
      {hasCompact ? (
        <>
          ...
          <div className="hidden min-w-0 flex-1 md:block">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-neutral-400">{t('search.editSearch')}</p>
          ...
          <div className="hidden min-w-0 flex-1 md:flex md:flex-[1.1] md:flex-col md:justify-center">
      ...
      <span
        className="flex h-10 shrink-0 items-center justify-center gap-1 rounded-full bg-[#1A56DB] px-3 text-white shadow-sm transition-colors group-hover:bg-[#1748b8] md:h-11 md:px-3.5"
        aria-hidden
      >
        <SearchRounded sx={{ fontSize: 20 }} />
        <span className="text-xs font-semibold">{t('search.edit')}</span>
      </span>
    </button>
  )
}
